class GistsController < ApplicationController
	def index
		gists = HTTParty.get("https://api.github.com/gists/public", :verify => false).parsed_response
		@gists = Array.new
		gists.each do |gist|
			formated_gist = Hash.new

			formated_gist["owner"] = gist["owner"].nil? ? "anonymous" : gist["owner"]["login"]
			formated_gist["description"] = gist["description"].nil? ? "no description" : gist["description"]
